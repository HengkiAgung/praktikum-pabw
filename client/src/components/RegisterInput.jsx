...
function InputRegister({ register }) {
  ...
  const [umur, onUmurChange] = useInput('');
  ...
  const onSubmit = (event) => {
    ...
    if (password === confirmPassword) {
      return register({ fullname, email, password, umur, role: "user" });
    }
  ...
  return (
    <div>
      <div>
          <label htmlFor="fullname" className="block text-sm font-medium text-gray-900 dark:text-white">Name</label>
      ...
      <div>
          <label htmlFor="umur" className="mt-10 block text-sm font-medium text-gray-900 dark:text-white">Umur</label>
          <input type="text" id="umur" value={umur} onChange={onUmurChange} className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="name@company.com"/>
      </div>
      <div>
